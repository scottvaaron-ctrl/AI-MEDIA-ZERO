...
def _entry_summary(entry: Any) -> str:
    ...
    if content and isinstance(content, list) and content[0].get("value"):
        text = strip_html(content[0]["value"])
    else:
...
def _split_wikipedia_onthisday(entry: Any) -> list[tuple[str, str]]:
    ...
    for li in items:
        ...
        if len(text) < 30:
            continue
        # first wiki link in the list item is the best article pointer
        m = re.search(r'href="(/wiki/[^"#]+)"', li)
        url = f"https://en.wikipedia.org{m.group(1)}" if m else entry.get("link", "")
        out.append((text[:200], url))
    return out
